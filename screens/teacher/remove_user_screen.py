...
class RemoveUserScreen:
    def __init__(self, screen):
        print("Iniciando RemoveUserScreen...")
        self.screen = screen
        self.running = True
        ...
        # Botões
        self.botao_remover = self.Botao(130, 380, 200, 40, 'Remover', self.remover)
    ...
    def remover(self):
        ...
        if all(dados):
            if os.path.isfile("usuarios.csv"):
                ...
                # Encontrar e remover a linha que contém exatamente os dados
                linhas_removidas = [linha for linha in linhas if linha != dados]

                if len(linhas_removidas) == len(linhas):
                    print("Usuário não encontrado para remoção.")
                else:
                    with open("usuarios.csv", "w", newline='', encoding='utf-8') as f:
                        writer = csv.writer(f)
                        writer.writerows(linhas_removidas)
            ...
            print("Preencha todos os campos para remover o usuário!")
    ...
    def run(self):
        print("Executando RemoveUserScreen.run()...")
        while self.running:
            for evento in pygame.event.get():
                ...
                self.botao_remover.handle_event(evento)
            ...
            titulo = self.fonte_titulo.render("Remover Usuário", True, self.PRETO)
            ...
            self.botao_remover.draw(self.screen)
...
# Teste do módulo diretamente
if __name__ == "__main__":
    pygame.init()
    screen = pygame.display.set_mode((800, 500))
    remove_user_screen = RemoveUserScreen(screen)
    result = remove_user_screen.run()
    print(result)
    pygame.quit()